...
export const addUsersToRoomMethod = async (userId: string, data: { rid: string; users: string[] }, user?: IUser): Promise<boolean> => {
	...
	await Promise.all(
		data.users.map(async (username) => {
			...
			// Check if validateFederatedUsername function exists before calling it
			if (typeof validateFederatedUsername === 'function') {
				...
				// Fallback: simple check for federated username format (@username:domain)
				const isFederatedFormat = sanitizedUsername.includes('@') && sanitizedUsername.includes(':');
				if (isFederatedFormat && !isRoomNativeFederated(room)) {
					throw new Meteor.Error('error-federated-users-in-non-federated-rooms', 'Cannot add federated users to non-federated rooms', {
						method: 'addUsersToRoom',
					});
				}
